...
struct Web_Unit : public virtual Unit {
    ...
    void init() override {
        ...
        x_handlers[listen_id] = [this](Context& ctx){
            ctx.node.value().type(int_id);
            ctx.node.value().size(4);
            ctx.node.value().set(ctx.root.owner().value().get());
            if(!ctx.node.scopes().empty()) {
                while(true) {
                    if(standard_travel_pass(ctx.node.scopes()[0],ctx.sub)) return;
                }
            }
        };

        x_handlers[accept_id] = [this](Context& ctx){
            int server_fd = *(int*)ctx.root.owner().value().get();
        ...
        x_handlers[response_id] = [this](Context& ctx){
            ...
            if(route_roles.hasKey(route_id)) {
                ...
                if(sessions.hasKey(token)) {
                    Node session = sessions.get(token);
                    std::string role = "";
                    for(int i=0;i<session.quals().length();i++) {
                        Node q = session.quals()[i];
                        if(q.type() == role_id) {
                            role = q.name().to_std();
                            break;
                        }
            ...
            if(method=="POST") {
                ...
                if(body_start != std::string::npos) {    
                    ...
                    ctx.sub->node = scan_for_node(target,ctx.sub->node.scopes()[0]);
                    if(ctx.sub->node.pool!=0) {
        ...
        x_handlers[port_id] = [this](Context& ctx) {
            ...
            if(ctx.node.children().length()>0) {
                port = *(int*)ctx.node.children()[0].value().get();
            }
            int server_fd = *(int*)ctx.root.owner().value().get();
    ...
    void run(Node server) override {
        start_stage(x_handlers);
        Context ctx;
        process_node(ctx,server);
    }
