...
namespace Acorn {
    ...
    static AcornCol* emitcol = nullptr;
    struct jump_request {
    ...
    struct Acorn_Dirt : public virtual Compiler_Unit {
     ...
     //AArch64 comparison codes
        ...
        static void syscall_append_to_buffer(uint64_t zero) {uint32_t z = zero; (*emitcol).push((void*)&z);}
        static void syscall_emit_movz_imm(uint64_t imm) {
            uint32_t instr =  MOVZ(2, imm & 0xFFFF, 0);
            (*emitcol).push((void*)&instr);
            if(imm > 0xFFFF) {
                uint32_t movk_instr = MOVK(2, (imm >> 16) & 0xFFFF, 16);
                (*emitcol).push((void*)&movk_instr);
            }
        }

        static void syscall_append_jump(uint64_t zero) {
            jump_request request;
            request.instr_idx = (uint32_t)(*emitcol).length()-1;
        ...
        static void syscall_resolve_terminator() {
            terminator_positions << (*emitcol).length(); //So we land just after the terminator
            for(int r=jump_requests.length()-1;r>=0;r--) {
                ...
                if(terminator_positions.length()>=request.desired_terminators) {
                    ...
                    uint32_t instr = B(offset);
                    (*emitcol).set(request.instr_idx, (void*)&instr);
