...
namespace Acorn {
    struct Acorn_Script : public virtual Compiler_Unit {
        ...
        string resolve_string_ticket(Node n) {
            if(is_live(n.value())) {
                if(is_live(n.value().data_ptr())&&!n.value().data_col().empty()&&is_live(n.getPtr())) {
                    return n.getString();
                } else {
                    Ptr p = get_ticket(name_store_id,1,char_id); 
                    CHECK_ERROR_VAL(deadptr,"Bad ticket in resolve_string_ticket");
                    n.value().set((void*)&p);
                    return (string&)p;
                }
            }
        ...
        Ptr resolve_ticket(Node n,  uint32_t size, uint32_t type, Ptr in = deadptr) {
            if(is_live(n.value())) {
                if(is_live(n.value().data_ptr())&&!n.value().data_col().empty()&&is_live(n.getPtr())) {
                    return n.getPtr();
                } else {
                    ...
                    CHECK_ERROR_VAL(deadptr,"Failed while creating new ticket for resolve ticket");
                    n.value().set((void*)&p);
                    return p;
                }
            }
        ...
        uint32_t init_property_id = reg_id("INIT_PROPERTY");
        ...
        list<list<uint32_t>> YAPAs = prep_YAPA_directory();
        ...
        uint32_t node_has_qual_id = overload_type(node_id,".'has_qual'(int)","NODE_HAS_QUAL",make_value(bool_id,1),[this](Context& ctx){
            ...
            uint32_t qualtype = ctx.node().right().getInt(0);
            CHECK_ERROR("Bad int in node_has_qual");
            bool b = n.has_qual(qualtype);
            ctx.node().value().set((void*)&b);
        });
        ...
        uint32_t string_append_id = overload_type(string_id,"+string","STRING_APPEND",make_value(string_id,sizeof(Ptr),0,char_id,1),[this](Context& ctx){
            ...
            std::string l = ctx.node().getString(0).to_std();
        ...
        void init() override {
            ...
            overload_type(string_id,list<std::string>{".'substr'(any)", ".'substr'(any,any)"},"STRING_SUBSTR",make_value(string_id,sizeof(Ptr),0,char_id,1),[this](Context& ctx){
                ...
                CHECK_ERROR("Bad args in string_substr");
            ...
            overload_type(string_id,list<std::string>{".'slice'(any,any)", ".'slice'(any,any,any)", ".'slice'(any,any,any,any)"},"STRING_SLICE",make_value(string_id,sizeof(Ptr),0,char_id,1),[this](Context& ctx){
                ...
                Node left = ctx.node().children()[0];
                Node right = ctx.node().children()[1];
                CHECK_ERROR("Bad args in string_slice");
            ...
            overload_type(string_id,list<std::string>{".'find'(any)", ".'find'(any,any)", ".'find'(any,any,any)"},"STRING_FIND",make_value(int_id,4),[this](Context& ctx){
                ...
                Node right = ctx.node().children()[1];
                Ptr ptr = ctx.node().getPtr(0);
                CHECK_ERROR("Bad args in string_find");
            ...
            add_function("paranoid_qual_kill",[this](Context& ctx){
                standard_sub_process(ctx);
                Context c = ctx.node().getContext(0);
                c.qual(deadptr);
            });
            ...
            add_function("capture_node",[this](Context& ctx){
                string output = resolve_string_ticket(ctx.node());
                Node n = deadptr;
                if(ctx.node().c0().type()==func_call_id) {
                    n = ctx.node().c0().scope().owner();
                } else {
                    standard_sub_process(ctx);
                    n = ctx.node().getNode(0);
                }
                output = n.name().to_std()+"_"+std::to_string(n.idx);
            },sizeof(Ptr),string_id);
            ...
            add_function("print_col",[this](Context& ctx){
                standard_sub_process(ctx);
                Ptr ptr = ctx.node().getPtr(0);
                print_column(resolve_to_col(ptr));
            });
            add_function("col_to_string",[this](Context& ctx){
                standard_sub_process(ctx);
                Ptr ptr = ctx.node().getPtr(0);
                string output = resolve_string_ticket(ctx.node());
            ...
            //The new form
                ...
                register_YAPA_type("Query",1,query_id);
            ...
            add_function("uscw",[this](Context& ctx){ //uspan_setup_crash_watcher
                setup_crash_watchers();
            });
            add_function("urcw",[this](Context& ctx){ //uspan_remove_crashwatcher
                for(int i=0;i<watchers.length();i++) {if(watchers[i].label=="crash") watchers.removeAt(i); break;}
            });
            ...
            x_handlers[to_unary_id(star_id)] = [this](Context& ctx){
                standard_sub_process(ctx);
                if(ctx.node().c0().value().size()!=sizeof(Ptr)) {throw_error("can't defrefrence because the node does not have storage for a Ptr"); return;}
                Ptr p = ctx.node().getPtr(0);
                CHECK_ERROR("Bad Ptr in unary star");
            ...
            x_handlers[literal_id] = [this](Context& ctx){
                ...
                if(vtype==int_id) {
                    ...
                    resolve_string_ticket(ctx.node()) = name;
                ...
                CHECK_ERROR("Failed to handle literal ",labels[vtype]," of name ",escape_string(name,true));
            };
            ...
            make_tokenized_keyword("hide_block",hide_block_id);
            x_handlers[hide_block_id] = [this](Context& ctx) {
                ctx.state(standard_travel_pass(ctx.node().scopes()[0],ctx.sub()));
            };
            ...
            add_function("correct_loaded_ptrs",[this](Context& ctx){
                ...
                Ptr p = ctx.node().getPtr(0);
                ColColCol& sub = resolve_to_subunit(p);
                offset_subunit_ptrs(sub,p.subunit,0);
            });

            add_function("diff_types",[this](Context& ctx){
                standard_sub_process(ctx);
                ColColCol& a = resolve_to_subunit(ctx.node().getPtr(0));
                ColColCol& b = types;

                uint64_t a_size = get_real_size_of_colcolcol(a);
                uint64_t b_size = get_real_size_of_colcolcol(b);
                print("A: ",fmem(a_size)," B: ",fmem(b_size));
                if((a_size-b_size)!=0) {
                    if(a_size>b_size) {
                        print("A is ",fmem(a_size-b_size)," larger than B");
                    } 
                    if(a_size<b_size) {
                        print("B is ",fmem(b_size-a_size)," larger than A");
                    }
                }
                
                ColCol& anodes = a[node_type_id];
                ColCol& bnodes = b[node_type_id];
                
                map<std::string,list<Node>> lookup_nodes_in_b_by_value;
                for(uint32_t i = 0; i < bnodes.length(); i++) {
                    Col& bcol = bnodes[i];
                    if(!bcol.live) continue;

                    Node n = Ptr(&b, node_type_id, i, 0);
                    if(is_live(n.value())) {
                        std::string key = Ptr_to_string(n.value(),n.value().cachelevel);
                        if(lookup_nodes_in_b_by_value.hasKey(key)) {
                            lookup_nodes_in_b_by_value.get(key).push(n);
                        } else {
                            lookup_nodes_in_b_by_value.put(key,{n});
                        }
                    }
                }
                ...
                print("New live values since snapshot: ");
                for(uint32_t i = 0; i < bvals.length(); i++) {
                    Col& bcol = bvals[i];
                    if(!bcol.live) continue;
                    ...
                    if(i >= avals.length()) {
                        ...
                        Col& acol = avals[i];
                        if(!acol.live && bcol.live) is_new = true;
                        else if(bcol.gen > acol.gen && bcol.live) is_new = true;
                    }
                    
                    if(is_new) {
                        Value v = Ptr(&b, value_type_id, i, 0);
                        print("  ",value_info(v));
                        if(is_live(v.type_scope())) {
                            print("  declared at: ",node_basic_info_with_position(v.type_scope()));
                        }
                        list<Node> empty_list;
                        list<Node> lookup = lookup_nodes_in_b_by_value.getOrDefault(Ptr_to_string(v,v.cachelevel),empty_list);
                        if(lookup.length()>0) {
                            for(uint32_t j=0;j<lookup.length();j++) {
                                print("LOOKUP ",j,": ");
                                print(node_to_string(lookup[j],2));
                            }
                        }
                    }
                }
            ...
            for(int YAPA_level=0;YAPA_level<YAPAs.length();YAPA_level++) {
                for(int y=0;y<YAPAs[YAPA_level].length();y++) {
                    uint32_t id = YAPAs[YAPA_level][y];
                    if(YAPA_level>1) {
                            ...
                            overload_type(id,list<std::string>{".'"+labels[lower_id]+"'(...)","."+labels[lower_id]+"(...)",".'"+labels[lower_id]+"'","."+labels[lower_id]},labels[id]+"_INIT_LOWER",make_value(lower_id,sizeof(Ptr)),[this,id,YAPA_level,lower_id](Context& ctx){
                                ...
                                if(!right.children().empty()) {
                                    for(int i=0;i<right.children().length();i++) {
                                        if(right.children()[i].type()==property_id) {right.children()[i].type(init_property_id);}
                                    }
                                }
                                standard_sub_process(ctx);
                                if(ctx.node().type()!=old_type) {standard_process(ctx); return;}
                                Ptr in = ctx.node().left().getPtr();
                                CHECK_ERROR("Bad args in ",labels[id]," init lower");
                                if(!right.children().empty()&&is_live(right.value().data_ptr())) {
                                    right.value().data_col().set(0,&deadptr);
                                }
                                CHECK_ERROR("Failed to clear in ",labels[id]," init lower");
                                Ptr push_to = deadptr;
                                if(is_YAPA_indirect.get(lower_id)) {
                                    push_to = resolve_ticket(right,sizeof(Ptr),ptr_id,in);
                                } else {
                                    push_to = resolve_ticket(right,sizeof(Ptr),lower_id,in); //<- at this point it's just add so not really a point to this
                                }
                                CHECK_ERROR("Failed to get push to in ",labels[id]," init lower");
                                YAPA_init(push_to,ctx.node().right(),is_YAPA_indirect.get(lower_id));
                                CHECK_ERROR("Failed to init in ",labels[id]," init lower");
                                ctx.node().value().set((void*)&push_to);
                            });
                        }
                    }
                }
            ...
            add_function("DEBUG_ROOT_UP",[this](Context& ctx){
                print(node_to_string(ctx.node().in_scope().owner(),0,0,0));
            });
            ...
            x_handlers[invoke_stage_id] = [this](Context& ctx){
                ...
                uint32_t type = left.type();
                if(ctx.node().children().length()>1) {
                    type = ctx.node().getInt(1);
                }
                ctx.node(left);
                //print("Invoking ",node_basic_info(ctx.node())," in ",stage->label," as ",labels[type]);
                stage->run(type)(ctx);
            };
        }
