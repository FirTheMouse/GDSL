    ...
    struct Tokenizer_Unit : public virtual Unit {
        ...
        size_t add_token(char c, const std::string& f) {
            ...
            tokenizer_functions[c] = [this,id,c](Context& ctx) {
                ctx.node = make_node();
                set_ptr(ctx.node,type_col,(void*)&id);
                resolve_to_col(resolve_to_ptr(ctx.node,name_col)).push(&c);
                set_ptr(ctx.node,x_col,(void*)&at_x);
                set_ptr(ctx.node,y_col,(void*)&at_x);
                ctx.result->push((void*)&ctx.node);
            };
